...
function inicio(){
    ...
    socios = JSON.parse(localStorage.getItem("listaSocios"));
...
function guardarSocio(){
    ...
    if (socios.length==0){
        ...
        socio.codSocio = parseFloat(socios[socios.length-1].codSocio) + 1;
    }
    ...
    localStorage.setItem("listaSocios", JSON.stringify(socios));
    actualizarBotones();
    cuadroErrores.innerHTML="";
...
function modificarSocio(){
    ...
    socio.dni = inputDNI.value;
    socio.categoria = inputCategoria.value;
    socio.codSocio = outPutCodSocio.value;
    socio.cuotaAlDia = socios[indice].cuotaAlDia;

    socios.splice(indice,1,socio);
